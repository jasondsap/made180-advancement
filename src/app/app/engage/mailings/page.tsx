  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        <SubTabs items={TABS.map((t) => ({ key: t.key, label: t.label, href: `/app/engage/mailings?tab=${t.key}` }))} active={active.key} />
        {canManage(ctx.role) && <Link href="/app/engage/mailings/new" style={btnPrimary}>New mailing</Link>}
      </div>
      ...
      <DataTable
        columns={columns}
        rows={messages}
        rowActions={(r) => <Link href={`/app/engage/mailings/${r.id}`} style={{ color: "var(--brand)", fontSize: ".85rem" }}>{r.status === "draft" ? "Edit" : "View"}</Link>}
        empty={<EmptyState icon="✉" title={`No ${active.label.toLowerCase()} yet`} description="Compose a letter and generate a print-ready PDF." action={canManage(ctx.role) ? <Link href="/app/engage/mailings/new" style={btnPrimary}>New mailing</Link> : undefined} />}
      />
    </div>
  );
}
