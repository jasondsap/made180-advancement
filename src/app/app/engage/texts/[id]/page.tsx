...
export default async function TextMessagePage({
  params,
  searchParams,
}: {
  ...
  if (!message || message.channel !== "sms") notFound();
  ...
  if (message.status === "draft" && canManage(ctx.role)) {
    ...
    return (
      <div>
        ...
        />
        <form action={deleteSmsMessageAction} style={{ marginTop: "1.5rem" }}>
  ...
  const [stats, recipients] = await Promise.all([statsForMessage(ctx.orgId, message.id), listRecipients(ctx.orgId, message.id)]);
  const columns: Column<EngageRecipient>[] = [
    { key: "to_phone", header: "Recipient", render: (r) => r.to_phone ?? "—" },
...
function Back() {
  return <p style={{ marginBottom: ".5rem" }}><Link href="/app/engage/texts" style={{ color: "var(--brand)", fontSize: ".88rem" }}>← Texts</Link></p>;
}
